---
name: vercel
description: Vercel platform deployment, Git integration (GitHub, GitLab, Bitbucket), preview deployments for pull requests, production deployments, environment variables management, custom domains configuration, project configuration (vercel.json/vercel.ts), monorepo deployment strategies, serverless functions (including Express), deployment protection, instant rollbacks, and production launch checklist. Applicable when deploying applications to Vercel, configuring preview/production environments, managing environment variables, setting up custom domains, or optimizing Vercel deployments.
---
