---
name: github-actions
description: GitHub Actions CI/CD pipeline configuration, workflow syntax, event triggers, job orchestration, matrix strategies, reusable workflows, composite actions, caching strategies (actions/cache), security hardening (secrets, permissions, GITHUB_TOKEN), self-hosted runners, environment protection rules, concurrency control, and monorepo optimization with Turborepo. Applicable when setting up CI/CD pipelines, automating build/test/deploy processes, optimizing workflow performance, or securing GitHub Actions workflows.
---
