...
description: Structured logging with Pino/Winston, log levels (error, warn, info, debug, trace), log redaction and sanitization, log aggregation (Datadog, Logtail, AWS CloudWatch), application performance monitoring (APM), health checks, metrics collection, alerting, and monitoring best practices for Express/Node.js and React applications. Applicable when implementing logging, setting up monitoring, configuring alerting, or debugging production issues.
---
